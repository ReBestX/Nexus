...
<head>
  ...
  <link rel="stylesheet" href="../../src/Draftposts.css">
        ...
        <?php
          if(isset($_GET['message']) && !empty(trim($_GET['message']))){
            ...
            if($message == "error1"){
              ...
              echo "<p class='post success' style='padding: 10px 20px; margin:0 0 15px 0; color:#198754; font-weight: 600; font-size:20px'>{$em}</p>";
            }
          }
        ?>
          <?php include "../includes/db.php";
          $query2 = "SELECT * FROM posts WHERE post_author = '{$user_id}' AND post_status = 'draft' ORDER BY post_id DESC";
          $select_all_posts_query = mysqli_query($connection, $query2);
          $postCount = mysqli_num_rows($select_all_posts_query);

          if($postCount > 0){
            echo "<div class='posts'>
                  <h1 class='post' style='padding: 20px; margin: 0 0 10px 0; font-size: 20px;'>Draft Posts</h1>";
            while($row = mysqli_fetch_assoc($select_all_posts_query)){
              ...
              echo "<div class='post' style='margin-bottom:15px'>
                      ...
                      <p class='lead'>
                        <span>by</span>
                        <a href='../Profile.php?id={$user_id}'>{$user_firstname} {$user_lastname}</a>
                      </p>
                      <p class='history'>
                      ...
                      <div class='postButtons'>
                        <div class='buttonCon' style='padding: 0 0 20px 0;'><div class='button'><a href='../EditDraftPost/{$row['post_id']}'><span>Edit Draft</span></a></div></div>
                        <div class='buttonCon'><div class='button'><a href='../../includes/UpdatePostStatus.php?id={$row['post_id']}'><span>Publish Post</span><i class='fa-solid fa-chevron-right'></i></a></div></div>
                      </div>
                    </div>";
            }
            echo "</div>";
            ...
            echo "<div class='posts'>
                    <p class='post' style='padding: 20px; margin: 0 0 10px 0; font-size: 20px;'>No Draft Posts</p>
                  </div>";
          }
          ?>
      </div>
